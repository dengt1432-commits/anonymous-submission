...
- **mid stage data mixture**
  ```yaml
    datasets:
      - json_path: ./data/llava_instruct/blip558k_stage1.5_finetune_w_prompt.json
        sampling_strategy: all
      - json_path: ./data/llava_instruct/coco118k_stage1.5_finetune_w_prompt.json
        sampling_strategy: all
      - json_path: ./data/llava_instruct/cc3m_recap_data_prompt_v2.json
        sampling_strategy: all
      - json_path: ./data/llava_instruct/ureader_tr_sft.json
        sampling_strategy: all
      - json_path: ./data/llava_instruct/instruct_azure_dc_zh_92K.json
        sampling_strategy: all
      - json_path: ./data/llava_instruct/allava/Evol-Instruct-GPT4-Turbo-143K.json
        sampling_strategy: all
      - json_path: ./data/llava_instruct/synthdog_zh/synthdog_zh_100k.json
        sampling_strategy: all
      - json_path: ./data/llava_instruct/synthdog_en/synthdog_en_100k.json
        sampling_strategy: all  
  ```
...
Our onevision stage data is available in [onevision-yaml](https://github.com/LLaVA-VL/LLaVA-NeXT/blob/main/scripts/train/onevision.yaml). The single-image portion can be downloaded from the above Huggingface link for onevision data. Here's a breakdown of each part:
  ...
  - Multi-image data is released in [M4-Instruct Data](https://huggingface.co/datasets/lmms-lab/M4-Instruct-Data). We combine the different subsets into two jsons (as they are mainly from DEMON and Mantis) in our training yaml, the jsons are:
    - ./data/llava_instruct/real_vision_flan/llava_ofa_DEMON-FULL_filtered_311085.json
    - ./data/llava_instruct/real_vision_flan/llava_ofa_mantis-instruct_reformatted.json
